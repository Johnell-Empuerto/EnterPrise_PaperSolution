...
{
    /// <summary>
    /// Generates Excel workbooks from FormDefinition (COM-based) or WorkbookDefinition (canonical).
    ///
    /// Phase 4.3: Added Generate(WorkbookDefinition, string) overload.
    /// The WbDef overload converts to FormDefinition internally and delegates to
    /// the COM-based generation. This keeps the conversion as an implementation
    /// detail — the external pipeline accepts WbDef directly.
    ///
    /// Target architecture: WorkbookDefinition → WorkbookGenerator → Excel
    /// </summary>
    public class WorkbookGenerator
    {
        ...
        /// <summary>
        /// Generate an Excel workbook from a WorkbookDefinition (canonical path).
        /// Converts to FormDefinition internally — no round-trip in the pipeline.
        /// </summary>
        public string Generate(WbDef.WorkbookDefinition wbDef, string outputPath)
        {
            _logger.LogInformation("[WBDF] Generating Excel from WorkbookDefinition: {Title} ({Sheets} sheets)",
                wbDef.Info?.Title, wbDef.Sheets.Count);

            // Convert to FormDefinition internally — this is an implementation detail
            // of the COM-based generator, not a pipeline step.
            var form = WbDef.WorkbookDefinitionConverter.ToFormDefinition(wbDef);

            // Add metadata so the COM generator can find the source XLSX path
            if (!string.IsNullOrEmpty(wbDef.SourcePath))
                form.Metadata["xlsxPath"] = wbDef.SourcePath;

            return Generate(form, outputPath);
        }
        ...
        public string Generate(FormDefinition form, string outputPath)
        {
            ...
            try
            {
                ...
                excelApp = new App { Visible = false, DisplayAlerts = false };
                ...
                // ═══ CP5: Read COM PageSetup BEFORE SaveAs ═══
                _logger.LogInformation("[X28][{ExportId}] ===== CP5: PageSetup BEFORE SaveAs =====", exportId);
                for (int cpi = 1; cpi <= workbook.Worksheets.Count; cpi++)
                {
                    var cpWs = (Excel.Worksheet)workbook.Worksheets[cpi];
                    LogPageSetup(exportId, $"CP5-S{cpi}({cpWs.Name})", cpWs);
                ...
                // ═══ CP6: Reopen saved file and read PageSetup ═══
                Checkpoint6ReopenCheck(exportId, outputPath);
        ...
        private string CalculatePrintArea(FormDefinition form)
        {
            ...
            if (form.Clusters != null)
            {
                foreach (var cluster in form.Clusters)
                {
                    ...
                    ParseAddressExtents(cluster.CellAddress, ref minRow, ref maxRow, ref minCol, ref maxCol);
                }
            }
            ...
            if (form.Sheets != null)
            {
                ...
                {
                    if (sheet.MergedCells == null) continue;
                    foreach (var mc in sheet.MergedCells)
                    {
                        if (string.IsNullOrWhiteSpace(mc.Address)) continue;
                        ParseAddressExtents(mc.Address, ref minRow, ref maxRow, ref minCol, ref maxCol);
                    }
                }
            }
        ...
        private static void ParseAddressExtents(string addr, ref int minRow, ref int maxRow, ref int minCol, ref int maxCol)
        {
            ...
            Dictionary<string, int> sheetIndexMap, string printAreaAddress, string exportId)
        {
            ...
            for (int si = 0; si < form.Sheets.Count; si++)
            {
                ...
                {
                    // ═══ CP3: Log model + COM BEFORE ApplyPageSettings ═══
                    _logger.LogInformation("[X28][{ExportId}] ===== CP3: BEFORE ApplyPageSettings sheet={Id} =====", exportId, sheetDef.Id);
                    LogPageSetup(exportId, $"CP3-BEFORE({sheetDef.Id})", ws, sheetDef.PageSettings);
                    ...
                    // ═══ CP4: Log COM AFTER ApplyPageSettings ═══
                    LogPageSetup(exportId, $"CP4-AFTER({sheetDef.Id})", ws);
                    _logger.LogInformation("[FORENSIC][{ExportId}]   ApplyPageSettings() SUCCESS", exportId);
                }

                // Apply PrintArea directly on the worksheet (more reliable than Names.Add)
                if (!string.IsNullOrEmpty(printAreaAddress))
                {
                    try
                    {
                        _logger.LogInformation("[FORENSIC][{ExportId}]   Setting ws.PageSetup.PrintArea={Addr}", exportId, printAreaAddress);
                        ws.PageSetup.PrintArea = printAreaAddress;
                        _logger.LogInformation("[FORENSIC][{ExportId}]   PrintArea set SUCCESS", exportId);
                    }
                    catch (Exception exPa)
                    {
                        _logger.LogWarning(exPa, "[FORENSIC][{ExportId}]   PrintArea set FAILED — fallback to SetDefinedNames", exportId);
                    }
                }
        ...
        // ════════════════════════════════════════════════════════════════════════
        private void LogPageSetup(string exportId, string label, Excel.Worksheet ws, PageSettings? modelPs = null)
        {
            try
            {
                var ps = ws.PageSetup;
                ...
                _logger.LogInformation(
                    "[X28][{ExportId}] [{Label}] COM: PA=[{PA}] CH={CH} CV={CV} " +
                ...
                {
                    _logger.LogInformation(
                        "[X28][{ExportId}] [{Label}] MDL: PA=(model) CH={CH} CV={CV} " +
                        "LM={LM:F4} RM={RM:F4} TM={TM:F4} BM={BM:F4} " +
                        "O={O} Z={Z} FW={FW} FH={FH}",
                        exportId, label,
                        modelPs.CenterHorizontally, modelPs.CenterVertically,
                        modelPs.LeftMargin, modelPs.RightMargin, modelPs.TopMargin, modelPs.BottomMargin,
                        modelPs.Orientation ?? "(null)", modelPs.Zoom, modelPs.FitToPagesWide, modelPs.FitToPagesTall);
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning("[X28][{ExportId}] [{Label}] LogPageSetup FAILED: {Msg}", exportId, label, ex.Message);
            }
        }

        // ════════════════════════════════════════════════════════════════════════
        // PHASE X.28 — TEMPORARY DIAGNOSTIC: Post-generation reopen check (CP6)
        // ════════════════════════════════════════════════════════════════════════
        private void Checkpoint6ReopenCheck(string exportId, string outputPath)
        {
            _logger.LogInformation("[X28][{ExportId}] ===== CP6: Reopening saved workbook for PageSetup verification =====", exportId);
            App? reopenApp = null;
            Workbook? reopenWb = null;
            try
            {
                reopenApp = new App { Visible = false, DisplayAlerts = false };
                reopenWb = reopenApp.Workbooks.Open(outputPath);
                for (int i = 1; i <= reopenWb.Worksheets.Count; i++)
                {
                    var ws = (Excel.Worksheet)reopenWb.Worksheets[i];
                    LogPageSetup(exportId, $"CP6-REOPEN-S{i}({ws.Name})", ws);
                    Marshal.ReleaseComObject(ws);
                }
                reopenWb.Close(SaveChanges: false);
            }
            catch (Exception ex)
            {
                _logger.LogError("[X28][{ExportId}] CP6 reopen FAILED: {Msg}", exportId, ex.Message);
            }
            finally
            {
                if (reopenWb != null) { try { Marshal.ReleaseComObject(reopenWb); } catch { } }
                if (reopenApp != null) { try { reopenApp.Quit(); Marshal.ReleaseComObject(reopenApp); } catch { } }
            }
            _logger.LogInformation("[X28][{ExportId}] ===== CP6 reopen complete =====", exportId);
        }
    }
}
