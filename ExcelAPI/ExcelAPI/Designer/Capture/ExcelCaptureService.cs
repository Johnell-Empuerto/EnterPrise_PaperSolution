    ...
    public class ExcelCaptureService : IExcelCaptureService
    {
        ...
        Workbook? workbook = null;
            ...
            try
            {
                ...
                // ── Stage 9: Build canonical WorkbookDefinition ────────
                try
                {
                    var wbDef = WbDef.WorkbookDefinitionConverter.FromCaptureResult(result, excelFilePath);
                    wbDef.SourcePath = excelFilePath;
                    wbDef.SourceFileName = Path.GetFileName(excelFilePath);

                    // Override sheet name with actual worksheet name from COM
                    if (worksheet != null && wbDef.Sheets.Count > 0)
                    {
                        wbDef.Sheets[0].Name = worksheet.Name ?? "Sheet1";
                    }

                    result.InternalWorkbookDefinition = wbDef;
                    _logger.LogInformation(
                        "[Stage 9/8] WorkbookDefinition built: {SheetCount} sheet(s), {FieldCount} field(s)",
                        wbDef.Sheets.Count,
                        wbDef.Sheets.Sum(s => s.Fields.Count));
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Failed to build WorkbookDefinition — continuing with CaptureResult only");
                }
        ...
        private void CleanupComObjects(Workbook? workbook, App? excelApp)
        {
            if (workbook != null)
            {
                try
